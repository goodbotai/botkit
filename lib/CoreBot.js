...
function Botkit(configuration) {
    ...
    botkit.restoreConversation = function(){}

    botkit.storeConversation = function(){
      var x = this.tasks.map(({convos}) => {
        return convos.map(({status, messages, thread, threads, startTime})=> {
          return {status, messages, thread, threads, startTime};
        })
      })

      botkit.debug('storing now!')
      client.hset('botkit:storage:conversations', 1458420534196460, JSON.stringify(x), redis.print)
    }
