...
var redis = new Redis();
const storageNs = "botkit:storage:conversations"
...
function Botkit(configuration) {
    ...
    function Conversation(task, message) {
        ...
        this.handle = function(message) {
            ...
            if (this.handler) {
                        ...
                        for (var p = 0; p < patterns.length; p++) {
                            if (patterns[p].pattern && botkit.hears_test([patterns[p].pattern], message)) {
                                botkit.middleware.heard.run(that.task.bot, message, function(err, bot, message) {
                                  if (typeof(patterns[p].callback)=='string') {
                                    patterns[p].callback = eval(patterns[p].callback)
                ...
                botkit.storeConversation(message);
            } else {
                // do nothing
            }
        };
    ...
    function Task(bot, message, botkit) {
        ...
        this.botkit = botkit;
        this.bot = bot;
        this.index = (message.user + Date.now()).toString();
        ...
        this.conversationEnded = function(convo) {
            ...
            // botkit.deleteConversation(convo)
            botkit.log(`Conversations are now ${this.convos.length}`)
            var actives = 0;
        ...
        this.taskEnded = function() {
            ...
            // botkit.deleteTask(this.source_message, this.index);
        };
    ...
    botkit.storeConversation = function(message) {
      var x = this.tasks.reduce((acc, { convos, index }) => {
        const result = convos.reduce(
          (
            acc,
            {
              status,
            ...
            if (newMessages.length>0) {
              newMessages =  newMessages.map((newMessage) => {
                if (newMessage.handler) {
                  if (!newMessage.handler) {
                    return ;
                  } else if (typeof(newMessage.handler)=='function') {
                    newMessage.handler = newMessage.handler.toString();
                  } else {
                    newMessage.handler = newMessage.handler.map(element => {
                      element.pattern = JSON.stringify(element.pattern)
                      element.callback = element.callback.toString()
            ...
            let newHandler
            if (newMessages.length>0 && newMessages[0]!==undefined) {
              newHandler = newMessages[0].handler
            } else {
              newHandler = null
            }
            if (context.user === message.user) {
              acc.push({
                status,
        ...
        if (result.length > 0) {
          acc.push({ index, convos: result });
        }
        return acc;
      }, []);
      botkit.debug('storing now!');
      redis
        .hset(storageNs, message.user, JSON.stringify(x))
        .catch(err => console.log(`Error while saving ${err}`));
    };

    botkit.restoreConversation = function(bot, message) {
      botkit.debug('Fetching conversations from Redis');
      return redis
        .hget(storageNs, message.user)
        .then(v => JSON.parse(v))
        .then(response => {
          if (response) {
            return response.map(({ index, convos }) => {
              var newTask = new Task(bot, message, this);
              newTask.index = index;
              convos.map(convo => {
                var newConvo = new Conversation(newTask, message);
                newConvo.id = botkit.convoCount++;
                [
                  newConvo.messages,
                  newConvo.threads,
                  newConvo.thread,
                ...
                newConvo.messages = newConvo.messages.map((message) => {
                  if (Array.isArray(message.handler)==true) {
                    message.handler = message.handler.map((element) => {
                      if (element.pattern) {
                        element.pattern = JSON.parse(element.pattern)
                      } else if(element.default) {
                        element.default = JSON.parse(element.default)
                      }
                      element.callback = eval(element.callback)
                      return element;
                    })
                  } else {
                    message.handler = eval(message.handler)
                  }
                  return message;
                })
                newConvo.handler = eval(convo.handler)
                newTask.convos.push(newConvo);
              });
              if (botkit.tasks.length > 0) {
                const thisTaskIndex = botkit.tasks.findIndex(
                  ({ index }) => index === newTask.index
                );
                botkit.tasks[thisTaskIndex] = newTask;
              } else {
                botkit.tasks.push(newTask);
              }
              botkit.debug(
                `Has pushed newTask and length is now ${botkit.tasks.length}`
              );
              return botkit;
            });
          } else {
            botkit.debug('This user does not have a saved conversation on Redis');
            return botkit;
          }
        })
        .catch(err => botkit.log(`Error while restoring conversations -> ${err}`));
    };
// This function is never called
    botkit.deleteConversation = function (conversation) {
      conversation.task.convos = conversation.task.convos.filter(({id}) => id!==conversation.id)
      botkit.log('Removing completed conversation and updating to Redis!')
      botkit.storeConversation(conversation.source_message)
    }
    // This function is never called. Will be called at Ln 914
    botkit.deleteTask = function(message,taskIndex) {
      botkit.tasks = botkit.tasks.filter(({index}) => index!==taskIndex)
      redis
        .hdel(storageNs, message.user)
        .then(response => {
          if (response) {
            botkit.debug(
              `The tasks of user ${message.user} have been deleted`
            );
          } else {
            botkit.debug(
              `The tasks of user ${message.user} could not be deleted`
            );
          }
        })
        .catch(error => botkit.log(error));
    };
    ...
    botkit.worker = function(botkit, config) {
        ...
        this.findConversation = function(message, cb) {
            botkit.debug('DEFAULT FIND CONVO in CoreBot');
            cb(null);
        };
    };
