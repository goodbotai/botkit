      ...
      var x = this.tasks.reduce((acc, { convos, index }) => {
        const result = convos.reduce(
          (
            ...
            if (newMessages.length>0) {
              newMessages =  newMessages.map((newMessage) => {
                if (newMessage.handler) {
                  if (message.handler) {
                    return ;
                  } else if (typeof(message.handler)=='function') {
            ...
            let newHandler = newMessages.length ? newMessages[0].handler ? newMessages[0].handler : null : null
          ...
          if (response) {
            return response.map(({ index, convos }) => {
              ...
              convos.map(convo => {
                ...
                if (Array.isArray(convo.handler)==true) {
                  newConvo.handler = convo.handler.map((handler) => {
                    handler.callback = eval(handler.callback)
                    return handler;
                  })
                } else {
